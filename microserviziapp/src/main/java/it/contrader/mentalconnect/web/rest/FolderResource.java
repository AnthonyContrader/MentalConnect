...
import it.contrader.mentalconnect.service.impl.FolderServiceImpl;
...
/**
 * REST controller for managing {@link it.contrader.mentalconnect.domain.Folder}.
 */
@RestController
@RequestMapping("/api")
public class FolderResource {
    ...
    private static final String ENTITY_NAME = "microserviziappFolder";
    ...
    public FolderResource(FolderServiceImpl folderService) {
     ...
     */
    @GetMapping("/folders")
    public ResponseEntity<List<FolderDTO>> getAllFolders(Pageable pageable) {
    ...
    @GetMapping("/foldersByUser/{id}")
    public ResponseEntity<List<FolderDTO>> getFoldersByUser(@PathVariable Long id) {
        log.debug("REST request to get a page of Folders by user");
        Pageable pageable = PageRequest.of(0, 20, Sort.by("id"));
        Page<FolderDTO> page = folderService.findAllByUser(id,pageable);
